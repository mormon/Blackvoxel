      ...
      if ((FabInfos = VoxelTable[VoxelType]->FabInfos))
        {
          ...
          for (TransNum = 0; TransNum < FabInfos->TransformationCount;
               TransNum++)
            {
              ...
              for (ResultNum = 0;
                   ResultNum < ZFabInfos::ZTransformation::Max_ResultVoxels;
                   ResultNum++)
                {
                  ...
                  if (Transformation->Result_VoxelType[ResultNum])
                    {
                      Out <<
                        "INSERT INTO `bv_fabinfos` (`Num`, `VoxelType`, `VoxelTypeName`, `VoxelTypePageNode`, `FabVoxelType`, `FabVoxelTypeName`, `FabVoxelTypePageNode`) VALUES ";
                      Out << "('" << Field_InfoNum;
                      Out << "', '" << Transformation->
                        Result_VoxelType[ResultNum];
                      Out << "', '" << VoxelTable[Transformation->
                                                  Result_VoxelType
                                                  [ResultNum]]->VoxelTypeName;
                      Out << "', '" << VoxelTable[Transformation->
                                                  Result_VoxelType
                                                  [ResultNum]]->
                        Documentation_PageNum;
                      Out << "', '" << VoxelType;
                      Out << "', '" << VoxelTable[VoxelType]->VoxelTypeName;
                      Out << "', '" << VoxelTable[VoxelType]->
                        Documentation_PageNum;
                      Out << "');\n";
                      ...
                      for (ResultNum2 = 0;
                           ResultNum2 <
                           ZFabInfos::ZTransformation::Max_ResultVoxels;
                           ResultNum2++)
                        {
                          if (Transformation->Result_VoxelType[ResultNum2])
                            {
                              Out <<
                                "INSERT INTO `blackvoxel`.`bv_fabresults` (`Num`, `InfoNum`, `VoxelType`, `ResultQuantity`, `VoxelTypeName`, `VoxelTypePageNode`) VALUES ";
                              Out << "('" << Field_FabResultNum;
                              Out << "', '" << Field_InfoNum;
                              Out << "', '" << Transformation->
                                Result_VoxelType[ResultNum2];
                              Out << "', '" << Transformation->
                                Result_Quantity[ResultNum2];
                              Out << "', '" << VoxelTable[Transformation->
                                                          Result_VoxelType
                                                          [ResultNum2]]->
                                VoxelTypeName;
                      ...
                      for (EntryNum = 0;
                           EntryNum < Transformation->EntryCount; EntryNum++)
                        {
                          ZFabInfos::ZFabEntry * Entry;
                          Entry = &Transformation->FabList[EntryNum];
                          ULong Voxel =
                            FabInfos->MaterialTable[Entry->Index].VoxelType;
                          ULong Quantity = Entry->Quantity;

                          Out <<
                            "INSERT INTO `blackvoxel`.`bv_fabelements` (`Num`, `InfoNum`, `Order`, `VoxelType`, `VoxelQuantity`, `VoxelTypeName`, `VoxelTypePageNode`) VALUES ";
  ...
  for (VoxelType = 0; VoxelType < 32767; VoxelType++)
    {
      if (!VoxelTable[VoxelType]->Is_NoType)
        {
          Out <<
            "INSERT INTO `bv_voxelname` (`VoxelType`, `VoxelName`, `MiningType`, `VoxelTypePageNode`) VALUES ";
          Out << "('" << VoxelType;
          Out << "', '" << VoxelTable[VoxelType]->VoxelTypeName;
...
bool
ZVoxelTypeManager::_Internal_CompareTransTables (ZString & Message,
                                                 ZFabInfos * Fab,
                                                 ZFabInfos::ZTransformation *
                                                 Tr1,
                                                 ZFabInfos::ZTransformation *
                                                 Tr2)
{
              ...
              if (Entry1->Quantity <= Entry2->Quantity)
                {
                  if (Entry1 < Entry2)
                    Ms << "{EXCHANGE}";
                  Ms << "[";
                  Ms << VoxelTable[Fab->MaterialTable[Entry1->Index].
                                   VoxelType]->VoxelTypeName;
...
void
ZVoxelTypeManager::FindFabConflics ()
{
  ...
  for (VoxelType = 0; VoxelType < 65536; VoxelType++)
    {
      if ((FabInfos = VoxelTable[VoxelType]->FabInfos))
        {
          printf ("Fab : %d\n", VoxelType);
          for (TransNum1 = 0; TransNum1 < FabInfos->TransformationCount;
               TransNum1++)
            {
              for (TransNum2 = 0; TransNum2 < FabInfos->TransformationCount;
                   TransNum2++)
                {
                  if (TransNum1 != TransNum2)
                    if (_Internal_CompareTransTables
                        (Message, FabInfos,
                         &FabInfos->TransformationTable[TransNum1],
                         &FabInfos->TransformationTable[TransNum2]))
                      {
                        printf ("> Conflict : %d vs %d %s\n", TransNum1,
                                TransNum2, Message.String);
                      }
...
Bool
ZVoxelTypeManager::LoadVoxelTypes ()
{
  ULong i, t;
  ...
  for (; i < 32768; i++)
    {
      switch (i)
        {
        case 49:
          VoxelType = new ZVoxelType_Storage (i);
          break;
        case 75:
          VoxelType = new ZVoxelType_UserTextureTransformer (i);
          break;
        case 89:
          VoxelType = new ZVoxelType_YellowGas (i);
          break;
        case 96:
          VoxelType = new ZVoxelType_PlaneZ1 (i);
          break;
        case 100:
          VoxelType = new ZVoxelType_LightTransmitter (i);
          break;

          // Mother Machine
        case 107:
          VoxelType = new ZVoxelType_FabMachine (i);
        ...
        case 108:
          VoxelType = new ZVoxelType_Programmable (i);
          break;
        case 114:
          VoxelType = new ZVoxelType_BlastFurnace (i);
          break;
        case 115:
          VoxelType = new ZVoxelType_FusionElement (i);
          break;
        ...
        case 158:
        case 159:
          VoxelType = new ZVoxelType_MiningRobot_xr1 (i);
          break;
        ...
        case 198:
          VoxelType = new ZVoxelType_Sequencer (i);
          break;
        case 204:
          VoxelType = new ZVoxelType_Picker (i);
          break;
        case 209:
          VoxelType = new ZVoxelType_Mover (i);
          break;
        ...
        case 210:
          ...
          break;
        case 212:
          VoxelType = new ZVoxelType_RTFM (i);
          break;
        case 213:
          VoxelType = new ZVoxelType_Egmy_T1 (i);
          break;
        case 214 ... 215:
          VoxelType = new ZVoxelType_SimpleButton (i);
          break;
        case 216 ... 231:
          VoxelType = new ZVoxelType_IndicatorLight (i);
          break;
        case 235:
          VoxelType = new ZVoxelType_Concrete (i);
          break;
        case 236:
        case 237:
          VoxelType = new ZVoxelType_ProgRobot_Asm (i);
          break;
        case 238:
          VoxelType = new ZVoxelType_Example (i);
          break;
